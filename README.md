...
## 1. Retrieval & Search (검색 스택)
* **문제:** 설계서(3.4)의 BGE-M3 Learned Sparse 초기 설정 검증 시, 한국어 질의에 Sparse를 혼합하면 Dense 단독 성능보다 오히려 하락함 (0.55 → 0.45).
* **원인:** 한국어 도메인 특성상 Learned Sparse의 토큰화 및 임베딩 효율이 저하됨.
* **해결:** Sparse 모델을 교체하는 대신 **질의 언어 분리 전략** 도입. (한국어 → Dense 전담 / 영어 번역 질의 → BM25 전담)
* **레슨:** 공개 리더보드 수치에 의존하지 않고, 실제 우리 데이터셋/질의 기반의 실측 검증이 필수적임.

## 2. Execution & State Safety (Graph & API 상한)
* **호출 상한 조정 (#29):**
  * **현상:** 번역 질의 추가(Ask 3회 → 4회)로 Fan-out 직후 API 호출이 ~95회까지 급증.
  * **문제:** 기존 설계서(5.1)의 종료 조건(100회) 여유가 없어 재시도 루프 실행 불가. (전체 실측 102회 발생)
  * **해결:** 상한치를 **150회**로 상향 조정하여 안정성 확보.

* **병렬 워커 예외 격리 (LangGraph State):**
  * **문제:** LangGraph 병렬 Fan-out 환경에서 특정 워커 예외 발생 시, 동일 Superstep 내 다른 정상 워커의 결과까지 State에서 유실됨 (1·2회차).
  * **원인:** 워커 내부 `try/except`만으로는 그래프 레벨의 State 전파 차단 불가.
  * **해결:** 그래프 레이어의 안전 래퍼(`graph/safe.py`)를 구축하여, 실패 발생 시 해당 워커 키의 "실패 기록"으로 State를 치환.
  * **성과:** 미구현 워커가 3개 존재하는 상태에서도 END 노드까지 완주하여 최종 PDF 출력 성공 (3회차).

## 3. Validation & Metrics (검증 규격 및 지표)
* **Validator 검증 완화 (#40):**
  * **문제:** 출처 태그 검증기(Validator) 규칙이 과도하게 엄격하여 다중 페이지 표기(`[논문 p.2, p.9]`)를 거부, 도메인/종합 워커 전체 실패 발생 (5회차).
  * **해결:** 출처 태그 규격 강제(장치 6)는 유지하되, 정밀 형식 검사에서 **태그 종류 유효성 검사**로 검증 수위를 완화.

* **한계점 산출 메트릭과 스키마 동기화:**
  * **현상:** 초기 `[추론]` 태그 비율 집계 시 100%(1/1)라는 왜곡된 수치 산출.
  * **원인:** 워커가 태그를 본문 문장이 아닌 `evidence[].tag` 객체 내에 할당하여 집계 로직 누락 발생.
  * **해결:** 집계 스키마를 두 형식 모두 탐색하도록 개편 ➔ 실제 비율 **5% (7/145)**로 정상화.
  * **레슨:** 정량적 한계점 지표는 워커 출력 스키마 설계 단계부터 정합성을 맞춰야 함.

## 4. Report Rendering & Citation Normalization (인용 정규화)
* **문제:** Raw Citation을 그대로 렌더링할 경우, Tavily가 수집한 arXiv 웹 페이지가 일반 '웹 출처'로 분류되거나, 동일 논문이 저자 미상 웹 링크 및 arXiv 중복 항목으로 분리 출력됨 (7회차: 11건 중 4건 중복/오분류).
* **해결:** 렌더링 전 정규화 파이프라인 구축 (`agents/report_report_render.py:normalize_citations`)
  1. **arXiv ID 추출 및 승격:** URL 내 arXiv ID 감지 시 '논문' 카테고리로 자동 승격.
  2. **중복 병합:** 동일 ID 건은 학회 정보가 포함된 메타데이터로 우선 병합.
  3. **메타데이터 보완:** 선정 논문은 `selection.yaml` 정보 활용, 풀 외 논문은 arXiv API로 저자/발행연도 보충.
  4. **웹 출처 정제:** 저자 미상 항목을 `기관명 (게시일 미상 · 접근일)` 규격으로 통일.
  * **성과:** 인용 건수 11건 ➔ 정제된 8건 (논문 3건, 웹 5건)으로 깔끔하게 일관성 확보. 출처 정제는 워커가 아닌 **보고서 렌더링 단계에서 일괄 처리**하는 것이 구조적으로 타당함.

## 5. 다음에 다르게 할 것
1. **프롬프트 언어 제약 명시:** 도메인 워커의 출력 언어를 프롬프트 수준에서 강제 (InfiniGen 판정 등 영문 출력 방지, #31).
2. **지표-스키마 동시 설계:** `[추론]` 비율, 재작성 효과 등 보고서에 표기될 주요 정량 지표는 초기 State 스키마 정의 시 함께 확정.
3. **E2E 통합 테스트 조기 수행:** 프로젝트 첫날 오전에 워커가 미완성된 상태더라도 전체 파이프라인 통합 실행을 최소 1회 수행. (안전 래퍼 덕분에 실행 가능하며, 병렬 예외 처리 이슈 등을 극도로 조기에 발견 가능했을 것)
