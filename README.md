...
- **validator 가 너무 엄격하면 근거가 있어도 버린다**: 출처 태그 검증이 `[논문 p.N]` 한 쪽만 허용해 `[논문 p.2, p.9]` 같은 실출력을 거부 → 도메인·종합 워커가 통째로 실패(5회차). 형식이 아니라 **태그 종류만** 검사하도록 완화 (#40). 장치 6(출처 태그 강제)은 유지
...
- **다음에 다르게 할 것**: ① 도메인 워커 출력 언어를 프롬프트에 고정(InfiniGen 판정이 영어로 나옴, #31) ② `[추론]` 비율·재작성 효과처럼 한계점에 들어갈 수치는 State 스키마 확정 때 같이 정한다 ③ 통합 실행을 첫날 오전에 한 번 돌린다 — 워커가 하나도 없어도 안전 래퍼 덕에 돌고, 형제 예외 문제를 그때 알았을 것이다
