...
- **검색**: 설계서 3.4 의 sparse 초기값(BGE-M3 learned sparse)은 실측에서 뒤집혔다 — 한국어 질의에 sparse 를 섞으면 dense 단독보다 나빠지고(0.55→0.45), 해법은 sparse 모델 교체가 아니라 **질의 언어 분리**(dense ← 한국어, BM25 ← 영어 번역)였다. 리더보드가 아니라 우리 질의로 재야 보이는 것 ([실측](experiments/sparse_compare/README.md))
- **호출 상한**: 설계서 5.1 종료 조건의 100회는 번역 질의 추가(ask 3→4회)로 fan-out 직후 ~95 에 닿아 재시도 루프가 돌 여유가 없었다 → **150** (#29). 전체 실행 실측 102회
- **워커 예외는 형제를 지운다**: LangGraph 병렬 fan-out 에서 워커 하나의 예외가 같은 슈퍼스텝의 다른 워커 결과까지 State 에서 사라지게 했다(통합 1·2회차). 워커 안의 `try/except` 만으로는 부족해 **그래프 층 안전 래퍼**(`graph/safe.py`)로 실패를 "자기 키의 실패 기록"으로 바꿨다 — 미구현 워커가 3개여도 END 까지 가서 PDF 가 나왔다(3회차)
- **validator 가 너무 엄격하면 근거가 있어도 버린다**: 출처 태그 검증이 `[논문 p.N]` 한 쪽만 허용해 `[논문 p.2, p.9]` 같은 실출력을 거부 → 도메인·종합 워커가 통째로 실패(5회차). 형식이 아니라 **태그 종류만** 검사하도록 완화. 장치 6(출처 태그 강제)은 유지
- **한계점 수치는 워커 출력 형식에 묶인다**: `[추론]` 비율이 처음엔 100%(1/1)로 나왔다 — 워커가 태그를 문장이 아니라 `evidence[].tag` 에 두기 때문. 집계를 두 형식 모두로 바꾸자 5% (7/145). "측정 가능한 한계점"은 스키마를 같이 설계해야 나온다
- **REFERENCE 는 citations 를 그대로 찍으면 안 된다**: 설계서 6장은 `citations` 스키마를 지정 형식으로 렌더링만 하면 된다고 봤지만, 7회차 REFERENCE 11건 중 4건이 Tavily 가 긁어 온 **arxiv.org 페이지가 "웹" 항목**으로 들어간 것이었다 — 선정 논문 2편이 `저자 미상(접근일). *제목*. arxiv.org, URL` 로 한 번 더 찍히는 중복까지. 렌더링 전에 **정규화 단계**를 넣었다(`agents/report_render.py` `normalize_citations`): URL 에서 arXiv id 를 잡으면 논문 항목으로 승격, 같은 id 는 학회 정보가 있는 쪽 하나로 병합, 선정 2편은 `selection.yaml` 의 저자·학회로, 풀 밖 논문은 arXiv API 로 저자·게시연도를 채우고 학회는 *arXiv* 로. 나머지 웹 항목도 `저자 미상` 대신 `기관명(게시일 미상 · 접근일)` 로. 결과 11건 → **8건**(논문 3 · 웹 5). 출처 형식은 워커가 아니라 **보고서 단계에서 한 번에** 맞추는 게 맞았다
- **다음에 다르게 할 것**: ① 도메인 워커 출력 언어를 프롬프트에 고정(InfiniGen 판정이 영어로 나옴) <br>② `[추론]` 비율·재작성 효과처럼 한계점에 들어갈 수치는 State 스키마 확정 때 같이 정한다 <br>③ 통합 실행을 첫날 오전에 한 번 돌린다 — 워커가 하나도 없어도 안전 래퍼 덕에 돌고, 형제 예외 문제를 그때 알았을 것이다
